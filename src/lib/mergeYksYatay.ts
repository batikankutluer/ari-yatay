...
function makeKey(faculty: string | undefined, program: string, isKktc: boolean): string {
  return [faculty ? normalizeText(faculty) : '', normalizeText(program), String(isKktc)].join('::');
}
...
function buildLookup(entries: ReadonlyArray<ProgramYksEntry>): Map<string, CompleteYearScores> {
  ...
  for (const entry of entries) {
    ...
    lookup.set(makeKey(entry.faculty, entry.program, entry.isKktc ?? false), normalizedScores);
    lookup.set(makeKey(undefined, entry.program, entry.isKktc ?? false), normalizedScores);
  }
